...
// Get variables
$carpark = $_GET['h-carpark'];
$vote = $_GET['h-vote'];
$filtered_hotels;

// Filter variables depending on get value input
if ($carpark && $vote) {
  $filtered_hotels = array_filter($hotels, function ($var) {
    return $var['parking'] == $_GET['h-carpark'] && $var['vote'] >= $_GET['h-vote'];
  });
} else if (!$carpark) {
  $filtered_hotels = array_filter($hotels, function ($var) {
    return $var['vote'] >= $_GET['h-vote'];
  });
} else {
  $filtered_hotels = array_filter($hotels, function ($var) {
    return $var['parking'] == $_GET['h-carpark'];
  });
};
?>
  ...
  <body>
    ...
    <main>
      <div class="container-md">
        ...
        <!-- Form -->
        <p>
          <a href="index.php#all-hotels">Return to full list</a>
        </p>
        ...
        <section class="hotels-table">
          ...
          <table class="table table-striped">
            ...
            <tbody class="table-group-divider">
              <?php foreach ($filtered_hotels as $hotelNum => $hotel) { ?>
              <tr class="text-center">
